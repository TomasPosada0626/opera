...
🚧 En **M2 - Inventario + Kardex**: `Warehouse` (bodegas), `Product`/`Category`/`Unit` (catálogo) y `StockMovement` (Kardex append-only, ver [ADR 0001](docs/adr/0001-kardex-append-only.md)) migrados. `GET /inventory/:productId/stock` calcula el stock actual (global y por bodega) sumando movimientos — nunca un campo editable. `POST /inventory/entradas` y `POST /inventory/salidas` ya registran movimientos (cualquier usuario autenticado — son operaciones de planta, no administrativas); salida usa una transacción `Serializable` para validar stock disponible sin condición de carrera entre salidas concurrentes; falta ajuste, filtros reutilizables y alertas de stock.
