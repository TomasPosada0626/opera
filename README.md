...
**Revisión y limpieza final de código, cobertura de tests en desktop (#62).** Código muerto real, no cosmético: `AUDIT_CHECKLIST.md` (nota de trabajo obsoleta de 2026-08-14, su propio encabezado decía borrarla — lo que documentaba, #87-92, llevaba cerrado desde entonces), `MAIN_DIST` en `electron/main.ts` (exportado, nunca referenciado, boilerplate del scaffold de `vite-plugin-electron`), `ts-loader`/`source-map-support` en el backend (confirmado con `depcheck` + grep: `nest-cli.json` compila con `tsc`, no webpack, nada requiere `source-map-support`), y dos ramas de `Modal.tsx` genuinamente inalcanzables — el botón "Cerrar" del header garantiza que siempre haya al menos un elemento enfocable, así que el guard de `focusable.length === 0` y el fallback `?? panelRef.current` nunca se ejecutan.

El desktop no tenía ninguna herramienta de cobertura instalada — se agregó `@vitest/coverage-v8` y midió una base de ~85/77/82/85% (statements/branches/functions/lines), muy por debajo del ~99.7% del backend (gateado al 95% en CI). El hueco estaba concentrado en código que solo se ejercita indirectamente en tests de pantalla con todo mockeado: el cuerpo real de los hooks de mutación/query (`mutationFn`/`queryFn`, nunca invocado — solo el hook mismo mockeado), utilidades de `lib/` (`api-client.ts`, `download-file.ts`, `auth-token.ts`), primitivos de UI (`Modal`, `ThemeToggle`, `UserMenu`), los siete componentes `RowActions` por entidad, y varios formularios/páginas con ramas sin probar. Se agregaron tests directos contra cada uno, reutilizando el patrón ya establecido en el repo (mock de `apiFetch` + `QueryClientProvider`) en vez de inventar uno nuevo. Resultado: 96.18/88/95.59/96.14% — la cobertura de branches quedó casi exactamente donde está la del backend (88.7%). `vite.config.mts` gana su propio `coverage.thresholds` (95/85/93/95, algo por debajo de lo medido para no romper por fluctuación normal) y un script `test:cov` que ahora corre en CI en vez del `test` plano — mismo gate de cobertura en ambos paquetes, no solo en el backend.

**Checkpoint de seguridad final de M6 (#80).** Cuatro auditorías en paralelo cubrieron todo el backend: auth/RBAC/usuarios/audit, inventario/producción, pedidos/remisiones/clientes/proveedores, y reportes/dashboard/búsqueda/catálogo — no solo auth, como pedía la issue. Tres volvieron limpias (nada por encima del umbral de confianza exigido). La de pedidos/remisiones encontró dos huecos reales en `RemissionsService.create()`: (1) nunca se validaba `order.status` antes de despachar — una remisión (SALIDA real de stock + registro de entrega al cliente) podía crearse contra un pedido que nunca llegó a `EN_ALMACEN`, incluido uno ya `CANCELADO`, mientras la bodega tuviera stock suficiente de ese producto por cualquier otra vía; y (2) el chequeo de "no exceder lo pendiente por línea" validaba cada entrada de `dto.items` por separado contra el mismo `remaining` previo a la transacción, así que repetir el mismo `orderItemId` dos veces en una sola solicitud (ej. dos líneas de 8 contra un pedido de 10) pasaba ambos chequeos individualmente y despachaba 16 en total. Ambos corregidos: `create()` ahora exige `EN_ALMACEN` de entrada, y las cantidades pedidas se suman por `orderItemId` antes de comparar contra lo restante — mismo patrón que ya usaba el chequeo de stock por producto. Los fixtures e2e de `remissions`/`reports` se ajustaron para reflejar el nuevo guard de estado.
