...
🚧 En **M3 - Producción**: schema de `BillOfMaterials`/`BillOfMaterialsItem` (receta) migrado — un producto terminado (`Product.type = FINISHED_GOOD`) tiene a lo sumo una receta activa, compuesta por líneas que consumen otros productos (materias primas/insumos) con una cantidad por unidad producida. No versionada/histórica a propósito: una vez completada, una orden de producción (#30) queda registrada como `StockMovement` real, no como una referencia a "qué receta se usó" — el ledger de movimientos es la fuente de verdad, no la receta vigente al momento. `RawMaterial` (#28) se cerró sin tabla nueva: ya está cubierto por el catálogo unificado `Product` desde M2. `ProductionOrder` (bodega, producto, cantidad, estado `PENDIENTE`/`EN_PROCESO`/`COMPLETADA`, `completedAt`) también migrado — declara la intención de producir N unidades; al completarse generará los `StockMovement` reales (eso lo hace #33, no la orden en sí). El método de costeo ya está decidido y documentado: promedio ponderado, no PEPS — encaja con el ledger append-only existente sin necesitar una estructura de lotes nueva (ver [ADR 0002](docs/adr/0002-costeo-promedio-ponderado.md)). `GET/POST /production-orders` ya funciona: crear una orden (ADMIN-only, mismo criterio corregido en inventario tras el `/security-review` de M2) valida que el producto sea `FINISHED_GOOD`, que tenga una receta activa, y que haya stock suficiente de cada componente según la receta × la cantidad pedida — sin reservar/bloquear stock todavía, eso es solo informativo hasta que #33 complete la orden con la misma protección `Serializable` que ya usan salidas y ajustes. Los `GET` (listado paginado y por id) son de lectura abierta, igual que el resto del catálogo. `POST /production-orders/:id/complete` ya funciona también: usa la receta **vigente** al completar (no una copia congelada de cuando se creó la orden), revalida stock dentro de una transacción `Serializable` (mismo patrón que salidas/ajustes), genera una `SALIDA` por cada componente consumido y una `ENTRADA` del terminado, y marca la orden `COMPLETADA` — todo atómico, así que no se puede completar la misma orden dos veces bajo carrera ni dejar movimientos a medias si algo falla. El costeo ya está implementado (#34): `StockMovement.unitCost` (opcional, solo en `ENTRADA` de inventario general) más `InventoryService.getAverageCost()`, que recorre el historial cronológico de un producto recalculando el promedio ponderado en cada entrada (una salida nunca cambia el promedio, solo reduce stock — no es una `SUM()`, a diferencia del stock, por eso necesita recorrer, no agregar). Al completar una orden, cada `SALIDA` de componente queda costeada al promedio vigente en ese momento, y la `ENTRADA` del terminado se costea a total consumido ÷ cantidad producida — `ProductionOrder.totalCost`/`unitCost` quedan grabados una sola vez, nunca recalculados después.
