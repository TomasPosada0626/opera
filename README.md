...
🚧 En **M2 - Inventario + Kardex**: `Warehouse` (bodegas), `Product`/`Category`/`Unit` (catálogo) y `StockMovement` (Kardex append-only, ver [ADR 0001](docs/adr/0001-kardex-append-only.md)) migrados. `GET /inventory/:productId/stock` calcula el stock actual (global y por bodega) sumando movimientos — nunca un campo editable; falta transacciones consistentes, endpoints de movimiento (entrada/salida/ajuste), filtros reutilizables y alertas de stock.
