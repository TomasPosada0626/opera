...
**Compras.** `Supplier`, `SupplierProduct` (precio vigente por par proveedor/producto) y `SupplierPurchase` (bitácora de qué se pidió, cuándo y a qué costo). Deliberadamente desconectado de `StockMovement` — la entrada real de materia prima al Kardex sigue siendo un movimiento manual; nada concilia automáticamente "lo pedido" contra "lo que entró" (gap conocido y documentado, no accidental).
...
| **Compras**         | Proveedores, precio por producto, bitácora de compras.                                                                                                                           |
