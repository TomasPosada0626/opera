...
🚧 En **M2 - Inventario + Kardex**: `Warehouse` (bodegas), `Product`/`Category`/`Unit` (catálogo) y `StockMovement` (Kardex append-only, ver [ADR 0001](docs/adr/0001-kardex-append-only.md)) migrados. `GET /inventory/:productId/stock` calcula el stock actual (global y por bodega) sumando movimientos — nunca un campo editable. `POST /inventory/entradas` ya registra entradas de inventario (cualquier usuario autenticado, no solo Administrador — son operaciones de planta, no administrativas); falta salida, ajuste, transacciones con isolation level, filtros reutilizables y alertas de stock.
